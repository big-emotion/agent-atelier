...
4. `scp` the tarball to `/tmp` on the VPS, and `scp`
   ...
   deployed a stale file (a lesson from the reference repo's early deploys).
...
- **Three environments**: a dedicated production App Service (no slots,
  ...
  slot sandbox (`workflow_dispatch` only). App names and hostnames are
  deployment coordinates — collect them from the operator at interview time;
  they are never recorded in this plugin.
